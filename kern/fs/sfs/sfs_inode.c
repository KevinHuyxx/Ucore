...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 YOUR CODE HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	...
	*/
    //处理起始的没有对齐到块的部分
    if ((blkoff = offset % SFS_BLKSIZE) != 0) {   //blkoff起始偏移未对齐
        size = (nblks != 0) ? (SFS_BLKSIZE - blkoff) : (endpos - offset);  //得到块的大小
        if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0) {  //找到内存文件索引对应的block的编号ino
            goto out;
        }
        if ((ret = sfs_buf_op(sfs, buf, size, ino, blkoff)) != 0) {  //读取数据
            goto out;
        }
        alen += size;  //实际读写长度相加
        if (nblks == 0) {   //只有这一个未对齐的块
            goto out;
        }
        buf += size, blkno ++, nblks --; //缓冲区大小更新，内存文件索引对应的block++，需要读的块减一个
    }

    size = SFS_BLKSIZE; //读取中间部分，这时候就要分解成块大小一次一次读
    while (nblks != 0) {
        if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0) {
            goto out;
        }
        if ((ret = sfs_block_op(sfs, buf, ino, 1)) != 0) { //读取数据
            goto out;
        }
        alen += size, buf += size, blkno ++, nblks --;//实际读写长度相加，缓冲区大小更新，内存文件索引对应的block++，需要读的块减一个
    }

    if ((size = endpos % SFS_BLKSIZE) != 0) {  //读取第三部分数据
        if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0) {
            goto out;
        }
