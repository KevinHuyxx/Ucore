...
load_icode(int fd, int argc, char **kargv) {
    ...
    struct Page *page;
    //(3.1) get the file header of the program (ELF format)
//
...
//
    ...
    //(3.2) get the entry of the program section headers of the program (ELF format)
//
...
//
    ...
    for (i = 0; i < argc; i ++) {
        argv_size += strnlen(kargv[i],EXEC_MAX_ARG_LEN + 1)+1;  //参数的总长度
    }

    uintptr_t stacktop = USTACKTOP - (argv_size/sizeof(long)+1)*sizeof(long);  //更新栈顶位置
    char** uargv=(char **)(stacktop  - argc * sizeof(char *));
    
    argv_size = 0;
    for (i = 0; i < argc; i ++) {
        uargv[i] = strcpy((char *)(stacktop + argv_size ), kargv[i]); //从低地址向高地址放参数
